...
public class PreventionController {
  ...
  @GetMapping()
  public ResponseEntity<ApiResponse<List<PreventionResponseDto>>> getPreventions() {
    return ResponseEntity.ok().body(ApiResponse.ok(preventionService.getPreventions()));
  }
  
  @Operation(summary = "방지시설 조회 API", description = "해당 방지시설의 상세정보를 조회합니다.")
  @GetMapping("/{preventionId}")
  public ResponseEntity<ApiResponse<PreventionDetailResponseDto>> getPrevention(@PathVariable Long preventionId) {
    return ResponseEntity.ok().body(ApiResponse.ok(preventionService.getPrevention(preventionId)));
  }
  
  @Operation(summary = "방지시설 수정 API", description = "해당 방지시설의 상세정보를 수정합니다.")
  @PatchMapping("/{preventionId}")
  public ResponseEntity<ApiResponse<PreventionResponseDto>> updatePrevention
      (
          @PathVariable Long preventionId,
          @Valid @RequestBody PreventionUpdateRequestDto request
      ) {
    return ResponseEntity.ok().body(ApiResponse.ok(preventionService.updatePrevention(preventionId, request)));
  }
  
  @Operation(summary = "방지시설 삭제 API", description = "방지시설 정보를 데이터베이스에서 삭제합니다.")
  @DeleteMapping("/{preventionId}")
  public ResponseEntity<ApiResponse<Void>> removePrevention(@PathVariable Long preventionId) {
